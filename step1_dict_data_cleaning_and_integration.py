...
if __name__ == "__main__":
    ...
    topic_data.columns = narration_data.columns = [
        '一级框架类型', '二级框架类型', '框架说明', '框架关键词', '编号', '类型'
    ]
    dictionary_data = pd.concat([topic_data[1:], narration_data[1:]],
                                axis=0).reset_index(drop=True)
    # ## 分词
    dictionary_comparison_table = pd.DataFrame()
    for index, class_name in dictionary_data['二级框架类型'].items():
        dictionary_comparison_table = pd.concat([
            dictionary_comparison_table,
            pd.DataFrame({
                class_name:
                safe_cut_words(dictionary_data.loc[index, '框架关键词'], '、')
            })
        ],
                                                axis=1)
