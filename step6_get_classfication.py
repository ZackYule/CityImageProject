...
if __name__ == "__main__":
    ...
    # 数据处理（单标签）
    dictionary_comparison_table_topic = dictionary_comparison_table.drop(
        ['积极判断框架', '消极判断框架', '事件框架', '细节框架'], axis=1)
    dictionary_comparison_table_narration = dictionary_comparison_table[[
        '积极判断框架', '消极判断框架', '事件框架', '细节框架'
    ]]
    data = get_single_classification(data,
                                     '议题关键词',
                                     dictionary_comparison_table_topic,
                                     inplace=True)
    data = get_single_classification(data,
                                     '叙事关键词',
    ...
    # 数据保存
    data.to_pickle(get_new_path(DataFilePath, '分类'))